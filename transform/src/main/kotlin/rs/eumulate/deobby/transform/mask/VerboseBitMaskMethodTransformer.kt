    ...
    override fun transform(item: MethodNode) {
        ...
        for (match in matches) {
            ...
            item.instructions[pushMask] = if (shift.opcode in LONG_SHIFT_OPCODES) {
                ...
                logger.debug { "Simplifying long shift of $mask to $simpleMask" }
                ...
                logger.debug { "Simplifying int shift of $truncated to $simpleMask" }
                simpleMask.toPushInstruction()
            }
        }

        logger.info { "Rewrote ${matches.size} bitmasks in ${item.name}${item.signature}" }
    }
